    ...
    >
        ...
        <div class="detail-cont" v-loading="loading">
            ...
            <div class="flex-item">
                ...
                <div>
                    <div style="text-align: center;">
                        <!-- :preview-src-list="[VUE_APP_API_BASE_URL + '/face/image/avatar?userId=' + params.userId]"
                            :src="VUE_APP_API_BASE_URL + '/face/image/avatar?userId=' + params.userId" -->
                        <el-image
                            style="width: 200px; height: 200px;margin-bottom: 16px;"
            ...
            <div v-if="params.id" style="margin-top: 16px;">
                <el-image
                            style="width: 100%;"
                            :preview-src-list="[getFaceReportSourceImageUrl(params.id)]"
                            :src="getFaceReportSourceImageUrl(params.id)"
                        >
                        </el-image>
            </div>
            <div class="bottom">
            <div class="button-box">
                <el-button-group>
                    <el-button type="primary" icon="el-icon-arrow-left" @click="nearlyFun(0)">{{$t('components.facedetail.6225p8')}}</el-button>
                    <el-button type="primary" @click="nearlyFun(1)">{{$t('components.facedetail.3udxki')}}<i class="el-icon-arrow-right el-icon--right"></i></el-button>
                </el-button-group>
                <div>
...
import {faceInfo,dowloadImage,faceNearly} from "../api"
export default {
    props:{
        currentId:{
            type:String,
    ...
    methods:{
        ...
        // 获取上一条、下一条
        async nearlyFun(type){
            const { data } = await faceNearly({reportId:this.detailId,type:type,...this.searchObj})
            if(data !='0' && data){
                this.detailId = data;
                this.getDetail()
            }else{
        ...
        getFaceReportImageUrl(id) {
            ...
            return `${VUE_APP_API_BASE_URL}/face/report/image?id=${id}&X-Token=${token}&t=`+new Date().getTime();
        },
        // 获取人脸报警原图图片
        getFaceReportSourceImageUrl(id) {
            const token = Cookies.get('X-Token');
            return `${VUE_APP_API_BASE_URL}/face/report/image?id=${id}&type=1&X-Token=${token}&t=`+new Date().getTime();
        },
        // 获取人脸头像图片
        getFaceAvatarImageUrl(id) {
            console.log('user id',id)
            const token = Cookies.get('X-Token');
            return `${VUE_APP_API_BASE_URL}/face/image/avatar?userId=${id}&X-Token=${token}&t=`+new Date().getTime();
        },
    }
};
</script>
